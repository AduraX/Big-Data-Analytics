...
Source = LOAD '/MscProj/weather' USING PigStorage('\t') AS (year:chararray, month:int, maxtemp:float, mintemp:float, frostdays:int, rainfall:float, sunshinehours:chararray);
...
STORE SortedReadings INTO '/MscProj/scrubbedweather' USING PigStorage(' ');